@extends('layouts.app')
@section('page_title', 'Nuevo Proyecto de Software')

@section('content')
<div class="container">
    <h2>Nuevo Proyecto de Software</h2>
    <form action="{{ route('proyectos_software.store') }}" method="POST">
        @csrf
        <div class="row g-3">
            <div class="col-md-6">
            ...
            <div class="col-md-4">
                ...
                <select name="tipo_software_id" class="form-select" required>
                    ...
                    @foreach($tipos as $t)
                        <option value="{{ $t->id }}" {{ old('tipo_software_id') == $t->id ? 'selected' : '' }}>{{ $t->nombre }}</option>
                    @endforeach
                </select>
            </div>
            <div class="col-md-4">
                ...
                <select name="responsable_id" class="form-select">
                    ...
                    @foreach($empleados as $e)
                        <option value="{{ $e->id }}" {{ old('responsable_id') == $e->id ? 'selected' : '' }}>{{ $e->nombre }}</option>
                    @endforeach
                </select>
            </div>
            <div class="col-md-4">
            ...
            <div class="col-md-4">
                ...
                <input type="text" name="stack" class="form-control" value="{{ old('stack') }}" placeholder="Ej: Python, React, SQL">
            </div>
            <div class="col-12">
                <label>Historial del proyecto (opcional)</label>
                <textarea name="historial" class="form-control">{{ old('historial') }}</textarea>
            </div>
